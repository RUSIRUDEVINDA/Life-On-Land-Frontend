...
const DashboardLayout = () => {
  ...
  const menuItems = [
    ...
    // { name: 'Protected Areas', path: '/dashboard/areas', icon: <MapPin size={18} /> },
    {
      ...
      path: "/dashboard/protected-areas",
      icon: <MapPin size={18} />,
    },
    ...
    <div className="flex h-screen w-full overflow-hidden bg-bg-soft">
      ...
      <aside className="w-55 bg-white border-r border-border-light flex flex-col z-10 transition-all duration-300">
        <div className="p-5 px-4 flex items-center gap-2">
          <div className="w-6 h-6 bg-primary-dark rounded-md flex items-center justify-center text-[12px] text-white">
            🌿
          </div>
          <h2 className="text-lg font-bold text-primary-dark m-0 tracking-tight">
            EcoTrack
          </h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="flex flex-col px-4 pr-3 pb-4 gap-1">
            <p className="text-[10px] font-semibold text-[#adb5bd] mb-1.5 mt-3 tracking-widest uppercase pl-2">
              MAIN MENU
            </p>
            {menuItems.map((item) => {
              const isActive =
              ...
              return (
                ...
                >
                  <span
                    className={`mr-3 flex items-center text-inherit opacity-80 ${isActive ? "text-primary opacity-100" : ""}`}
                  >
                    {item.icon}
                  </span>
                  <span className="flex-1">{item.name}</span>
                  {item.name === "Map Tracking" && (
            ...
            <button
              className="flex items-center text-[13px] font-medium py-2 transition-all duration-200 relative bg-transparent border-none cursor-pointer text-left w-full hover:text-primary-dark text-text-gray"
        ...
        <div className="p-4">
          <p className="text-[9px] color-[#adb5bd] text-center">
            EcoTrack v1.0 BETA
          </p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden bg-bg-soft">
        <header className="h-14 bg-transparent flex justify-between items-center px-6 z-5">
          <div className="flex items-center bg-white px-3 py-1.5 rounded-2xl w-70 shadow-[0_1px_4px_rgba(0,0,0,0.02)]">
            ...
            />
            <div className="bg-bg-soft border border-border-light text-text-gray text-[9px] px-1.5 py-0.5 rounded-md font-semibold">
              ⌘F
            </div>
          </div>
